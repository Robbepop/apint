// The functions should not be split up more.
...
fn large_div_by_small(duo: &mut [Digit], ini_duo_sd: usize, div: &mut Digit) {
    ...
    let (mut quo,mut rem) = duo[ini_duo_sd].wrapping_divrem(*div);
    duo[ini_duo_sd] = quo;
    for duo_sd_sub1 in (0..ini_duo_sd).rev() {
        let duo_double = DoubleDigit::from_lo_hi(duo[duo_sd_sub1],rem);
        let tmp = duo_double.wrapping_divrem((*div).dd());
...
fn large_div_by_u32(duo: &mut [Digit], ini_duo_sd: usize, div: &mut Digit) {
    ...
    let tmp = duo_double.wrapping_divrem(dd(div_u32));
    let mut quo_lo = lo(tmp.0);
    let mut rem_lo = lo(tmp.1);
    ...
    *div = Digit(u64::from(rem_lo));
}

// Note: This will be refactored to not use macros in the future. Macros are used for being able to
// change the temporary buffer types easily

/// Sets the `$array` to be the two's complement of itself, all the way up to `$len` (exclusive)
...
/// Assigns `$sum + $sub` to `$target` (`sub` is intended to be the two's complement of some value),
...
macro_rules! special1 {
    ($sum_len:expr,$sum:ident,$sub:ident,$target:ident,$val:expr,$add_len:expr,$add:ident) => {{
        let (tmp, mut carry) = $sum[0].carrying_add($sub[0]);
        $target[0] = tmp;
        ...
        $target[$sum_len-1] = $sum[$sum_len-1]
            ...
            .wrapping_add(carry);
        // second assignment
        let (tmp, mut carry) = $add[0].carrying_add($val);
        $sum[0] = tmp;
        for i0 in 1..$add_len {
            if carry == Digit::zero() {
            ...
            let tmp = $add[i0].carrying_add(carry);
            $sum[i0] = tmp.0;
            carry = tmp.1;
        }
...
fn mul_or_mulsub1_algorithm (
    len: usize,
    duo: &mut [Digit],
    duo_sd: usize,
    duo_sig_dd: DoubleDigit,
    div: &mut [Digit],
    div_sig_dd: DoubleDigit,
    quo: &[Digit]
) {
    ...
    let sub_len = sub.len();
    // The case where `-(div * mul)` overflows the digits availiable in the slices is handled
    // automatically here
    let mut b = true;
    if carry == Digit::zero() {
...
/// Special long division algorithm core. This adds `mul << bits` to `quo`, and subtracts
/// `(mul * div) << bits` from `duo`.
/// Assumptions:
///  - ini_duo_sd > 0
///  - div_sd == 0
fn special_long_division_core(
    bits: usize,
    duo: &mut [Digit],
    duo_sd: usize,
    duo_sig_dd: DoubleDigit,
    div: &mut [Digit],
    div_sig_d_add1: DoubleDigit,
    quo: &mut [Digit]
) {
    ...
    // addition unrolling hell instead of long multiplication unrolling hell.
    let mul = duo_sig_dd.wrapping_div(div_sig_d_add1);

    // add `mul << bits` to `quo`
    // no unrolling hell here because `bits_ll < n` and it takes a shift of `n` to overflow
    let split_mul = mul << bits_ll;
    let (tmp, mut carry) = split_mul.lo().carrying_add(quo[digits]);
    quo[digits] = tmp;
    let tmp = split_mul.hi().dd()
    ...
    for i in (digits+2)..quo.len() {
        ...
        let tmp = quo[i].carrying_add(carry);
        quo[i] = tmp.0;
        carry = tmp.1;
    }

    // Subtracts `(mul * div) << bits` from `duo`.
    // `mul` can be stored in a `Digit` plus one bit. The first block handles if that extra
    // bit is not set, and the second block handles if the bit is set.
    if mul.hi() == Digit::zero() {
        let mul = mul.lo();
        // carry for bits that wrap across digit boundaries when `<< bits_ll` applied
        let (tmp0, mut wrap_carry) = (div[0].dd() << bits_ll).lo_hi();
        // the regular multiplication carry
        let (tmp1, mut mul_carry) = mul.dd().wrapping_mul(tmp0.dd()).lo_hi();
        // this carry includes the two's complement increment carry
        let (tmp2, mut add_carry) = (!tmp1).dd()
        ...
        //       | the first column is this loop prelude (the `x` is here because there is no
        //       | previous carry), and the each of the next columns is a new iteration of the loop
        //       v
        //   2222x <- mul_carry
        //    7987 <- div
        ...
        //  1111xx <- add0_carry
        //   23961 <- tmp0
        //   7987  <- div shifted up by one digit (to handle the high bit in `mul`)
        // +______
        //  103831 <- tmp1
        //
        // add to duo with the negated tmp1 (with the carry from the two's complement
        // being put into `wrap_carry`) and shifted (with `wrap_carry`)

        let mul = mul.lo();
        let (tmp0, mut mul_carry) = mul.carrying_mul(div[0]);
        let tmp1 = tmp0;
        let mut add0_carry = Digit::zero();
        //the increment from the two's complement can be stored in `wrap_carry`
        let (tmp2, mut wrap_carry) =
            ((!tmp1).dd().wrapping_add(Digit::one().dd()) << bits_ll).lo_hi();
        let (tmp3, mut add1_carry) = tmp2.carrying_add(duo[digits]);
        duo[digits] = tmp3;
...
fn large_div_by_large(
    ...
    // initial difference between the places of the most significant bits
    let ini_diff_sb = ini_duo_sb - div_sb;

    // An unrolled `mul` or `mul - 1` branch is not used here unlike in `specialized_div_rem`.
    ...
    let mut duo_sig_dd;
    let quo_potential = (ini_diff_sb / Digit::BITS) + 1;
    ...
    loop {
        ...
        if div_lesser_bits <= duo_lesser_bits {
            let bits = duo_lesser_bits - div_lesser_bits;
            special_long_division_core(bits, duo, duo_sd, duo_sig_dd, div, div_sig_d_add1, &mut quo);
        } else {
        ...
        // `quo` should have 0 added to it branch
        if div_sb > duo_sb {
            //quotient = `quo`
            //remainder = `duo`
        ...
        // `quo` should have 0 or 1 added to it branch
        if duo_sb == div_sb {
            let place = duo_sd + 1;
            // if `div <= duo`
        ...
        if duo_sd < 2 {
            ...
            div[0] = rem.lo();
            div[1] = rem.hi();
            let (tmp, mut carry) = quo[0].carrying_add(mul.lo());
            duo[0] = tmp;
            let tmp = quo[1].dd()
            ...
            for i0 in 2..quo.len() {
                ...
                let tmp = quo[i0].carrying_add(carry);
                duo[i0] = tmp.0;
                carry = tmp.1;
            }
            return
...
///   if b && !rhs.is_zero() {
///       lhs.wrapping_dec();
///       // Some more operations can be added here, based on if the new remainder or a euclidean
///       // modulo is needed
///   }
/// 
...
impl ApInt {
    ...
    /// Dividing function. `duo` is divided by `div`, the quotient is assigned to `duo`, and the
    /// remainder is assigned to `div`. `false` is returned if division by zero happened. Nothing is
    /// modified in the case of division by zero.
    pub(crate) fn algorithm_divrem(duo: &mut [Digit], div: &mut [Digit]) -> bool {
        ...
        // Special cases are aggressively taken care of throughout this function, both because
        ...
        match div.iter().rposition(|x| x != &Digit::zero()) {
            // `div_sd` is the most significant nonzero `div` digit, which never changes
            Some(div_sd) => {
                // `ini_duo_sd` is the initial most significant nonzero duo digit
                ...
                if div_sd == 0 {
                    ...
                    if ini_duo_sd == 0 {
                        let tmp = duo[0].wrapping_divrem(div[0]);
                        duo[0] = tmp.0;
                        div[0] = tmp.1;
                        return true
                    }
                    if (div[0].leading_zeros() as usize) >= (Digit::BITS / 2) {
                        ...
                        // faster than `large_div_by_small`
                        large_div_by_u32(duo,ini_duo_sd, &mut div[0]);
                        return true
                    } else {
                        // regular long division branch
                        large_div_by_small(duo, ini_duo_sd, &mut div[0]);
                        return true
                    }
                }
                ...
                // quotient is 0 branch (but `ini_duo_sb == div_sb` is not checked yet)
                ...
                // `ini_duo_sd` is 1 branch
                // ini_duo_sd cannot be 0 or 1 for `large_div_by_large`
                if ini_duo_sd == 1 {
                    let tmp = DoubleDigit::from_lo_hi(duo[0], duo[1])
    ...
    pub fn wrapping_udivrem_assign(lhs: &mut ApInt, rhs: &mut ApInt) -> Result<()> {
        match ApInt::zip_access_data_mut_both(lhs, rhs)? {
            ZipDataAccessMutBoth::Inl(duo,div) => {
                if *div != Digit::zero() {
                    let tmp = duo.wrapping_divrem(*div);
                    *duo = tmp.0;
                    *div = tmp.1;
                    return Ok(())
                }
...
// Note: these tests only work if multiplication and a few other functions work
#[cfg(test)]
mod tests {
    use super::*;
    ...
    /// This is mainly to determine that signed division is negating properly, and that errors are
    /// correct.
    #[test]
    fn small() {
        /// Does all of the small division tests.
        /// 
        ...
        macro_rules! s {
            ($signed:expr,$fun_assign:ident,$fun_into:ident,$r0:expr,$r1:expr,$r2:expr,$r3:expr/*,$div_op:ident*/) => {
                ...
                match ApInt::from(12345678912345689123456789123456789u128).*/
                {
                    ...
                    tmp.$fun_assign(&rhs).unwrap();
                    assert_eq!(tmp, ApInt::from($r0));
    ...
    /// Shifts small numbers up, for easier debugging.
    #[test]
    fn large_shifts() {
        let resize = [
        ...
        for (i, _) in resize.iter().enumerate() {
            ...
            let tmp = product.clone().into_wrapping_udiv(&lhs).unwrap();
            if tmp != rhs {
                panic!("lhs_shl:{:?}\nrhs_shl:{:?}\nlhs:{:?}\nrhs:{:?}\n={:?}\ntemp:{:?}",lhs_shl[i],rhs_shl[i],lhs,rhs,product,tmp);
            }
    ...
    /// This tests a number of problematic cases found during development
    #[test]
    fn special() {
        assert_eq!(
            ApInt::from(123u8)
