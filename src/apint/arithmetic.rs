...
impl ApInt {
    ...
    // TODO: add tests
    #[allow(dead_code)]
    pub(crate) fn overflowing_uadd_assign(
        &mut self,
        rhs: &ApInt,
    ) -> Result<bool> {
    ...
    // TODO: add tests
    #[allow(dead_code)]
    pub(crate) fn overflowing_sadd_assign(
        &mut self,
        rhs: &ApInt,
    ) -> Result<bool> {
        ...
        match self.zip_access_data_mut_self(rhs)? {
            ...
            Ext(lhs, rhs) => {
                ...
                let rhs_sig_nonzero: usize =
                    match rhs.iter().rposition(|x| !x.is_zero()) {
                        Some(x) => x,
                        None => {
                ...
                let lhs_sig_nonzero: usize =
                    match lhs.iter().rposition(|x| !x.is_zero()) {
                        Some(x) => x,
                        None => {
                            for x in lhs.iter_mut() {
                                x.unset_all()
                ...
                match (lhs_sig_nonzero == 0, rhs_sig_nonzero == 0) {
                    (false, false) => {
                        let lhs_sig_bits = (lhs_sig_nonzero * Digit::BITS)
                            + (Digit::BITS
                                - (lhs[lhs_sig_nonzero].leading_zeros()
                                    as usize));
                        let rhs_sig_bits = (rhs_sig_nonzero * Digit::BITS)
                            + (Digit::BITS
                                - (rhs[rhs_sig_nonzero].leading_zeros()
                                    as usize));
                        ...
                        if tot_sig_bits <= (lhs.len() * Digit::BITS) {
                            ...
                            let mut sum = Vec::with_capacity(
                                lhs_sig_nonzero + rhs_sig_nonzero + 2,
                            );
                            sum.push(temp.0);
                            ...
                            let temp = mult.carrying_mul_add(
                                rhs[rhs_sig_nonzero],
                                mul_carry,
                            );
                            sum.push(temp.0);
                            ...
                            for lhs_i in 1..lhs_sig_nonzero {
                                ...
                                for rhs_i in 1..rhs_sig_nonzero {
                                    let temp0 = mult.carrying_mul_add(
                                        rhs[rhs_i], mul_carry,
                                    );
                                    mul_carry = temp0.1;
                            ...
                            let mut mul_carry = temp0.1;
                            let temp1 =
                                sum[lhs_sig_nonzero].carrying_add(temp0.0);
                            ...
                            for lhs_i in 1..sig_nonzero {
                                ...
                                let rhs_i_upper =
                                    sig_nonzero.wrapping_sub(lhs_i);
                                // middle digits of this row
                                for rhs_i in 1..rhs_i_upper {
                                    let temp0 = lhs[lhs_i].carrying_mul_add(
                                        rhs[rhs_i], mul_carry,
                                    );
                                    mul_carry = temp0.1;
                                    let temp1: DoubleDigit = sum[lhs_i + rhs_i]
                    ...
                    (true, false) => {
                        ...
                        if rhs_sig_nonzero == lhs.len() - 1 {
                            lhs[rhs_sig_nonzero] = mult.wrapping_mul_add(
                                rhs[rhs_sig_nonzero],
                                mul_carry,
                            );
                        } else {
                    ...
                    (false, true) => {
                        ...
                        if lhs_sig_nonzero == lhs.len() - 1 {
                            lhs[lhs_sig_nonzero] = rhs[0].wrapping_mul_add(
                                lhs[lhs_sig_nonzero],
                                mul_carry,
                            );
                        } else {
                            let temp = rhs[0].carrying_mul_add(
                                lhs[lhs_sig_nonzero],
                                mul_carry,
                            );
...
impl ApInt {
    ...
    #[inline]
    pub(crate) fn aarons_algorithm_divrem(
        duo: &mut [Digit],
        div: &mut [Digit],
    ) -> bool {
        ...
        fn large_div_by_u32(
            ...
            let (mut quo_hi, mut rem_hi) =
                wrapping_divrem(hi(duo[ini_duo_sd]), div_u32);
        ...
        fn large_div_by_large(
            ...
            let ini_duo_sb = (ini_duo_sd * Digit::BITS)
                + (Digit::BITS - (ini_duo_lz as usize));
            ...
            let div_sig_d = if div_lz == 0 {
                ...
                (div[div_sd] << div_lz)
                    | (div[div_sd - 1] >> (Digit::BITS - div_lz))
            };
            ...
            loop {
                duo_lesser_bits = (Digit::BITS - (duo_lz as usize))
                    + (Digit::BITS * (duo_sd - 2));
                ...
                if duo_lesser_bits >= div_lesser_bits {
                    ...
                    let (digits, bits_ll) =
                        (bits / Digit::BITS, bits % Digit::BITS);
                    ...
                    let (temp, mut carry) =
                        split_mul.lo().carrying_add(quo[digits]);
                    quo[digits] = temp;
                    let temp = split_mul
                        .hi()
                        .dd()
                    ...
                    if mul.hi().is_zero() {
                        ...
                        // bits_ll` applied
                        let (temp0, mut wrap_carry) =
                            (div[0].dd() << bits_ll).lo_hi();
                        ...
                        let (temp2, mut wrap_carry) =
                            ((!temp1).dd().wrapping_add(Digit::ONE.dd())
                                << bits_ll)
                                .lo_hi();
                        let (temp3, mut add1_carry) =
                            temp2.carrying_add(duo[digits]);
                        duo[digits] = temp3;
                        for i in (digits + 1)..=duo_sd {
                            let temp0 = mul
                                .dd()
        ...
        match div.iter().rposition(|x| !x.is_zero()) {
            Some(div_sd) => {
                ...
                let ini_duo_sd: usize =
                    match duo.iter().rposition(|x| !x.is_zero()) {
                        Some(x) => x,
                        None => {
                    ...
                    (false, false) => {
                        ...
                        if ini_duo_sd == 1 {
                            let temp = DoubleDigit::from_lo_hi(duo[0], duo[1])
                                .wrapping_divrem(DoubleDigit::from_lo_hi(
                                    div[0], div[1],
                                ));
                            duo[0] = temp.0.lo();
                            duo[1] = temp.0.hi();
    ...
    /// the case of division by zero.
    #[inline]
    pub(crate) fn aarons_algorithm_remdiv(
        duo: &mut [Digit],
        div: &mut [Digit],
    ) -> bool {
    ...
    pub fn wrapping_udivrem_assign(
        lhs: &mut ApInt,
        rhs: &mut ApInt,
    ) -> Result<()> {
    ...
    /// - If `lhs` and `rhs` have unmatching bit widths.
    /// - If division by zero is attempted
    pub fn wrapping_uremdiv_assign(
        lhs: &mut ApInt,
        rhs: &mut ApInt,
    ) -> Result<()> {
        match ApInt::zip_access_data_mut_both(lhs, rhs)? {
    ...
    /// - If `lhs` and `rhs` have unmatching bit widths.
    /// - If division by zero is attempted
    pub fn wrapping_sdivrem_assign(
        lhs: &mut ApInt,
        rhs: &mut ApInt,
    ) -> Result<()> {
        if rhs.is_zero() {
    ...
    /// - If `lhs` and `rhs` have unmatching bit widths.
    /// - If division by zero is attempted
    pub fn wrapping_sremdiv_assign(
        lhs: &mut ApInt,
        rhs: &mut ApInt,
    ) -> Result<()> {
        if rhs.is_zero() {
...
mod tests {
    ...
    mod inc {
        ...
        fn test() {
            assert_eq!(
                ApInt::from(14u8).into_wrapping_inc(),
                ApInt::from(15u8)
            );
            assert_eq!(
                ApInt::from(15u8).into_wrapping_inc(),
                ApInt::from(16u8)
            );
            assert_eq!(
                ApInt::from(16u8).into_wrapping_inc(),
                ApInt::from(17u8)
            );
            assert_eq!(
                ApInt::from(17u8).into_wrapping_inc(),
                ApInt::from(18u8)
            );
            assert_eq!(
    ...
    mod wrapping_neg {
        ...
        fn test_vals() -> impl Iterator<Item = i128> {
            [
                0_i128, 1, 2, 4, 5, 7, 10, 42, 50, 100, 128, 150, 1337, 123123,
        ...
        fn range() {
            for v in test_vals() {
                assert_symmetry(
                    ApInt::from_i8(v as i8),
                    ApInt::from_i8(-v as i8),
                );
                assert_symmetry(
                    ApInt::from_i16(v as i16),
                    ApInt::from_i16(-v as i16),
                );
                assert_symmetry(
                    ApInt::from_i32(v as i32),
                    ApInt::from_i32(-v as i32),
                );
                assert_symmetry(
                    ApInt::from_i64(v as i64),
                    ApInt::from_i64(-v as i64),
                );
    ...
    mod mul {
        ...
        fn rigorous() {
                ...
                for rhs_nine in 0..num_u8 {
                    ...
                    'outer: for lhs_nine in 0..num_u8 {
                        ...
                        for i in 0..nines_after_eight {
                            if 1 + zeros_after_one + nines_before_eight + i
                                >= num_u8 - 1
                            {
                                continue 'outer
                            }
            ...
            let resize = [
                7usize, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128,
                129, 137, 200, 255, 256, 700, 907, 1024, 2018, 2019,
            ];
            let lhs_shl = [
                0usize, 1, 0, 1, 4, 7, 4, 10, 13, 0, 31, 25, 7, 17, 32, 50, 0,
                64, 249, 8, 777, 0, 1000, 0,
            ];
            let rhs_shl = [
                0usize, 0, 1, 1, 3, 6, 4, 14, 10, 0, 0, 25, 0, 18, 32, 49, 100,
                64, 0, 256, 64, 900, 1000, 0,
            ];
            for (i, _) in resize.iter().enumerate() {
                ...
                let rhs = ApInt::from(11u8)
                    ...
                    .unwrap();
                let zero = ApInt::from(0u8)
                    .into_zero_resize(BitWidth::new(resize[i]).unwrap());
                let one = ApInt::from(1u8)
                    .into_zero_resize(BitWidth::new(resize[i]).unwrap());
        ...
        fn complex() {
            ...
            assert_eq!(
                ApInt::from([1u64, 18446744073692774368, 268435456])
                    .into_wrapping_add(&ApInt::from([
                        0u64,
                        1,
                        18446744073709519359
                    ]))
                    .unwrap()
                    .into_wrapping_udiv(&ApInt::from([
                        0u64,
            ...
            let lhs_shl = [
                0usize, 1, 0, 1, 4, 6, 4, 10, 13, 0, 31, 25, 7, 17, 32, 50, 0,
                64, 249, 8, 777, 0, 900, 0,
            ];
            let rhs_shl = [
                0usize, 0, 1, 1, 3, 5, 4, 14, 10, 0, 0, 25, 0, 18, 32, 49, 100,
                64, 0, 256, 64, 900, 1000, 0,
            ];
            for (i, _) in resize.iter().enumerate() {
                let lhs = ApInt::from(5u8)
                ...
                assert_eq!(
                    zero.clone().into_wrapping_udiv(&lhs).unwrap(),
                    zero
                );
                assert_eq!(
                    zero.clone().into_wrapping_udiv(&rhs).unwrap(),
                    zero
                );
                ...
                assert_eq!(
                    product.clone().into_wrapping_udiv(&rhs).unwrap(),
                    lhs
                );
                assert_eq!(
                    zero.clone().into_wrapping_urem(&lhs).unwrap(),
                    zero
                );
                assert_eq!(
                    zero.clone().into_wrapping_urem(&rhs).unwrap(),
                    zero
                );
                ...
                assert_eq!(
                    product.clone().into_wrapping_urem(&lhs).unwrap(),
                    zero
                );
                assert_eq!(
                    product.clone().into_wrapping_urem(&rhs).unwrap(),
                    zero
                );
                assert_eq!(
                    product
    ...
    mod megafuzz {
        ...
        fn pull_request_35_regression() {
            ...
            let v1 =
                ApInt::from((1u128 << 64) | (7u128)).into_zero_resize(width);
            ...
            let v3 = ApInt::from([0u64, 0, 17179852800, 1073676288])
                ...
                .into_wrapping_mul(
                    &ApInt::from(1u128 << 115).into_zero_resize(width),
                )
                .unwrap();
            assert_eq!(
                v3,
        ...
        fn identities(
            ...
            let rand_width =
                BitWidth::new((random::<usize>() % size) + 1).unwrap();
            ...
            let anti_overflow_mask = if tot_leading_zeros < size {
                if rhs.leading_zeros() == 0 {
                    ...
                    ApInt::unsigned_max_value(BitWidth::from(
                        rhs.leading_zeros(),
                    ))
            ...
            if rhs != *zero {
                ...
                if !temp0.overflowing_uadd_assign(&rem).unwrap() {
                    ...
                    ApInt::wrapping_udivrem_assign(&mut temp0, &mut temp1)
                        .unwrap();
        ...
        fn fuzz_random(size: usize, iterations: usize) {
            ...
            for _ in 0..iterations {
                ...
                let ones =
                    ApInt::unsigned_max_value(BitWidth::new(r0).unwrap())
                        .into_zero_extend(width)
                        .unwrap();
